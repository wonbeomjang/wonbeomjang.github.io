...
- 이 사이트는 아래 5개를 **의도적 override**로 유지한다. 지우지 말 것.
...
- `_layouts/`는 **비어 있어야 한다.** v1.2 이관 과정에서 `_layouts/default.html`을 삭제했다. 이 파일은 아주 오래된 al-folio 네이밍(`head.html`, `scripts/jquery.html` 등, 존재하지 않는 include)을 참조한 채 `_layouts/default.liquid`에 가려져 있던 죽은 파일이었고, gem 이관으로 `default.liquid`가 사라지자 전면에 나서면서 빌드를 깨뜨렸다. 다시 만들지 말 것.
