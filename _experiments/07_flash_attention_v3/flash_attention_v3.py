...
FA3 논문의 7가지 핵심 기법 중 Triton으로 표현 가능한 것만 적용:
  1. (논문 §3.2) Producer-consumer warp specialization → ✗ Triton 표현 불가
...
본 구현이 FA2 대비 추가하는 것:
  ...
  (d) 1D grid (num_m_tiles · bh,) — 인접 program_id가 같은 (bh, m) 묶음을 받게 분배해
     L2 친화적 SM 매핑 유도
  (e) Backward는 FA2 그대로 재사용 (FA3 backward 개선은 wgmma 의존)
...
def flash_attention_v3(q, k, v, causal=False, return_lse=False):
    """FA3 forward.
...
def flash_attention_v3_autograd(q, k, v, causal=False):
    ...
    return FlashAttentionV3Function.apply(q, k, v, causal)
...
def main():
    ...
    print()
    print("--- 벤치마크: FA1 vs FA2 vs FA3 vs PyTorch (causal, head_dim=64) ---")
    num_heads, head_dim = 16, 64
    seq_lengths = [256, 512, 1024, 2048, 4096]

    fa1_ms, fa2_ms, fa3_ms, torch_ms = [], [], [], []
    ...
    try:
        ...
        rows = []
        for n, t1, t2, t3, tp in zip(seq_lengths, fa1_ms, fa2_ms, fa3_ms, torch_ms):
            rows.append([
                n, f"{t1:.4f}", f"{t2:.4f}", f"{t3:.4f}", f"{tp:.4f}",
                f"{t2/t3:.2f}x", f"{tp/t3:.2f}x",
            ])
        print(tabulate_fn(
            rows,
            headers=["Seq", "FA1 (ms)", "FA2 (ms)", "FA3 (ms)", "PyTorch (ms)",
                     "FA3/FA2", "FA3/PT"],
            tablefmt="github",
        ))
    except ImportError:
        for n, t1, t2, t3, tp in zip(seq_lengths, fa1_ms, fa2_ms, fa3_ms, torch_ms):
            print(f"  seq={n}: FA1={t1:.4f}ms  FA2={t2:.4f}ms  FA3={t3:.4f}ms  "
                  f"PT={tp:.4f}ms  FA3/FA2={t2/t3:.2f}x  FA3/PT={tp/t3:.2f}x")
