...
6. **논문 링크는 제목 + venue 표기**: `> [Full Paper Title](arxiv_url) (First Author et al., Affiliation, Venue Year)` 형식으로 첫 줄에 배치
   - 예: `> [Crescendo: Multi-Turn LLM Jailbreak Attack](https://arxiv.org/abs/2404.01833) (Russinovich et al., Microsoft, USENIX Security 2025)`
   - venue 표기 규칙:
     - 채택된 학회/저널: `USENIX Security 2025`, `ICLR 2026`, `NeurIPS 2024`, `CVPR 2023`, `EMNLP Findings 2024` 등
     - preprint/미발표: `arXiv 2024` (연도는 arxiv 최초 공개 연도)
     - 테크니컬 리포트: `Tech Report 2024` 또는 소속 + `Tech Report YYYY`
   - 저자는 **first author + et al.** 형식, 단독 저자면 `et al.` 생략
   - 소속은 주요 소속 하나만 (`Microsoft`, `Google DeepMind`, `Meta AI`, `OpenAI`, `Anthropic`, `SKT` 등)
   - **불확실한 정보는 적지 않는다**: venue·소속·저자가 명확히 확인되지 않으면 그 항목은 생략한다 (예: 소속만 불확실하면 `(Author et al., Venue Year)`로, venue만 불확실하면 `(Author et al., Affiliation, arXiv YYYY)`로). 추측한 정보를 적느니 비우는 것이 낫다.
7. **시리즈 글은 상호 링크**: 관련 포스트를 마지막에 인용구로 안내
8. **일상 비유로 직관 잡기**: 어려운 개념마다 일상 비유를 덧붙여 처음 보는 독자도 따라오게 한다
9. **토이 예제로 step-by-step**: 알고리즘/방법은 작은 가상 예제(작은 어휘·짧은 시퀀스 등)로 한 스텝씩 직접 따라가게 한다
10. **수식은 기호 하나씩 풀이**: 수식을 던진 뒤 각 기호가 무엇을 뜻하는지 한 줄씩 설명한다
11. **"무엇을"보다 "왜"**: 동작뿐 아니라 왜 그렇게 하는지(설계 의도)를 설명한다

## 수식 표기 (필수)

al-folio는 Kramdown + MathJax를 쓴다. **인라인·디스플레이 구분 없이 모든 수식을 `$$...$$`로 감싼다.**

- single `$...$`는 **금지**. Kramdown이 math로 인식하지 않아, 수식 내부의 `*`, `_`가 마크다운 강조로 처리되어 렌더링이 깨진다. 인라인 변수도 `$$x_{1:n}$$`처럼 쓴다.
- 본문 속 "달러(돈)" 기호는 `\$`로 이스케이프한다. 단, ` ``` ` 코드블록 안의 `$`는 그대로 둔다.
- **수식 내부 `\_` 금지**: `$$...$$` 안에서는 `\_` 대신 그냥 `_`를 쓴다 (`\text{coverage\_bonus}` ❌ → `\text{coverage_bonus}` ✅). Kramdown이 `\_`를 보존해 MathJax가 백슬래시를 그대로 렌더링한다.
- **번호 매김 리스트 안 inline `$$...$$`에서 bare `|` 금지**: 조건부 확률·집합 크기 등은 `\mid` / `\lvert ... \rvert`를 사용한다 (`\log\pi(x|z)` ❌ → `\log\pi(x \mid z)` ✅). 리스트 안 `|`는 Kramdown이 테이블 셀 구분자로 해석해 수식이 두 컬럼으로 쪼개진다.
- 커밋 전 점검: `grep -nP '(?<!\$)\$(?!\$)' <파일>` 결과가 (이스케이프된 `\$` 제외) 비어 있어야 한다.
