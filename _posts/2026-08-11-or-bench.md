...
이 트레이드오프의 구조는 [#10 Overoptimization](/blog/2026/reward-model-overoptimization/)이 지적한 Goodhart 문제와도 맞닿아 있다 — 하나의 지표(여기서는 안전 프롬프트 거절률)만 밀어붙이면, 측정되지 않던 다른 축(유해 프롬프트 통과)이 조용히 무너진다. [#41 프론티어 모델의 reward 설계](/blog/2026/frontier-reward-design/)에서 다룰 최신 모델들이 "거절이 아니라 안전한 완수(safe completion)를 보상"하는 방향으로 옮겨가는 것도, 결국 이 Spearman 0.878이라는 숫자가 가리키는 트레이드오프를 정면으로 겨냥한 설계다.
