...
| 2   | Inter-warpgroup ping-pong (GEMM↔softmax) | ~15–20%                 | △ (`tl.async_task` 시범)                     | 미적용       |
| 3   | TMA (비동기 메모리 복사)                 | H100에서 10–15%         | △ (`tl.make_tensor_descriptor` experimental) | 미적용       |
...
_FWD_CONFIGS_V3 = [
    ...
    # BLOCK_M=256 (head_dim 작을 때만 SRAM 충분)
...
def _early_prune(configs, named_args, **kwargs):
    head_dim = named_args.get("HEAD_DIM", 64)
    seq_len = named_args.get("seq_len", 1024)
    ...
    for c in configs:
        ...
        if bm > seq_len:
            continue
        # Q + K + V (fp16) + acc (fp32)
        sram = (bm + 2 * bn) * bd * 2 + bm * bd * 4
        if sram > 144 * 1024:  # A100 SM당 ~164KB, num_stages buffer 고려해 144KB 컷
            continue
        pruned.append(c)
    if not pruned:
        return [triton.Config({"BLOCK_M": 64, "BLOCK_N": 32}, num_stages=3, num_warps=4)]
    return pruned

@triton.autotune(
    configs=_FWD_CONFIGS_V3,
    key=["seq_len", "HEAD_DIM", "IS_CAUSAL"],
    prune_configs_by={"early_config_prune": _early_prune},
)
@triton.jit
def _fa3_fwd_kernel(...):
    ...
...
```python
@triton.jit
def _fa3_persistent_fwd_kernel(...):
    pid = tl.program_id(0)
    grid_size = tl.num_programs(0)

    num_m_tiles = tl.cdiv(seq_len, BLOCK_M)
    total_tiles = num_bh * num_m_tiles

    for tile_id in range(pid, total_tiles, grid_size):
        pid_bh = tile_id // num_m_tiles
