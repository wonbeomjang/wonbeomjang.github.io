...
- 그래서 **"굳이 RM을 학습시키지 않아도 되는 경우"를 먼저 걸러내는 것**이 설계의 1단계다([#33](/blog/2026/deepseek-r1/)).
...
| ------------------------------- | ----------------------------------------- | -------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------- |
| ① 규칙 기반 verifiable reward   | 정답을 프로그램으로 판정 가능 (수학·코드) | 파라미터 0 → hacking 불가, 학습 비용 0 | 검증 가능한 도메인에만 적용        | [#33](/blog/2026/deepseek-r1/)                                                   |
...
| ③ reference 기반 judge          | 정답 예시 + 채점 기준(rubric)이 있을 때   | 해석 가능, 기준을 명시적으로 주입      | 판정 모델 추론 비용, judge 편향    | [#34](/blog/2026/prometheus-2/)                                                  |
| ④ generative RM (self-critique) | 기준은 있으나 정답 예시가 없을 때         | 근거를 생성 → 검증·디버깅 가능         | 추론 비용 큼, judge도 뚫릴 수 있음 | [#38](/blog/2026/deepseek-grm-spct/), [#43](/blog/2026/one-token-to-fool-judge/) |
...
**정책 최적화(KL 정규화, [#20](/blog/2026/ppo/)):**
...
"정답을 프로그램으로 판정 가능한가"가 첫 갈림길이다. 가능하다면 학습된 RM을 **쓰지 않는 것**이 정답이다([#33](/blog/2026/deepseek-r1/)). 규칙 기반 reward를 짤 때 실무 포인트는 두 가지다.
...
- **여러 RM을 평균낸다.** 여력이 되면 [#14 WARM](/blog/2026/warm-weight-averaged-reward/)처럼 weight averaging으로 RM 하나의 특이한 취약점을 상쇄한다.
