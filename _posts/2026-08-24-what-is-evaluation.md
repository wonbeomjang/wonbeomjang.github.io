...
이 식을 $$k$$에 대해 미분하면(또는 극단값을 대입하면) 방향이 바로 드러난다. $$\bar{r} \in (0, 1)$$을 고정한 채 $$k \to \infty$$이면 $$\alpha \to 1$$이다. 실제로 $$\alpha$$는 $$\bar{r}$$가 조금이라도 양수인 한 $$k$$에 대해 단조증가한다 — $$k$$를 늘리는 것만으로 평균 상관이 전혀 오르지 않아도 $$\alpha$$는 계속 오른다.
...
문항이 4개일 때는 "그럭저럭"으로 보이던 $$\alpha = 0.632$$가, 같은 유형의 문항을 8배로 늘리는 것만으로 $$\alpha = 0.932$$까지 오른다. 그런데 이 표 어디에도 "문항이 구성개념을 더 폭넓게 대표하게 됐다"는 정보는 없다 — $$\bar{r} = 0.30$$이라는 문항간 유사도는 처음부터 끝까지 그대로다. 벤치마크 제작자가 "우리 벤치마크는 $$\alpha$$가 0.93이나 되니 신뢰도가 매우 높다"고 홍보한다면, 그 문장이 감추고 있는 것은 "문항 32개가 사실상 같은 유형을 반복한 결과일 수 있다"는 가능성이다.

**이것이 함정이다.** 벤치마크 문항 수를 100개에서 1,000개로 늘리면 $$\alpha$$는 거의 항상 오른다. 그런데 새로 추가한 900개 문항이 기존 100개와 거의 동일한 유형(예: 같은 패턴의 사칙연산 응용 문제)이라면, 늘어난 것은 **내용의 폭이 아니라 같은 것을 반복 측정하는 횟수**뿐이다. 높은 $$\alpha$$가 "이 벤치마크는 일관되게 무언가를 잰다"는 것은 보장하지만, "그 무언가가 구성개념을 폭넓게 대표한다"는 것은 전혀 보장하지 않는다. 내적일관성이 높으면서 내용 타당도는 좁은, 겉보기에만 그럴듯한 벤치마크가 이렇게 만들어진다.
