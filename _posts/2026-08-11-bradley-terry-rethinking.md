...
**가정 1 — 진짜 품질은 흔들리지 않는다.** 응답 $$(x,y)$$에는 오라클 효용 $$r_{x,y}$$가 존재하고, 이 값은 **결정론적**이다. 즉 "이 답이 저 답보다 낫다"는 사실 자체는 고정되어 있다.
...
$$\mathbb{1}(\cdot)$$은 안의 조건이 참이면 1, 거짓이면 0인 지시함수다. 이 식이 말하는 것은 **개별 annotator는 동전을 던지지 않는다**는 것이다. 그 사람은 자기가 보는 값을 기준으로 확실하게 고른다.
