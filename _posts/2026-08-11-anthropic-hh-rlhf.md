...
이 계산이 장난감 숫자만은 아니다. 실제로 논문 Figure 1을 보면 52B online HH RLHF 모델은 baseline(context-distilled 52B) 대비 harmlessness 선호 빈도가 약 80%, helpfulness Elo는 약 250점 격차로 나타난다. 반면 harmlessness 없이 helpfulness만으로 online 학습한 모델은 helpfulness Elo가 약 330점까지 올라가 전문 작가 밴드에 근접하지만, harmlessness 선호 빈도는 baseline보다도 낮은 약 27%로 떨어진다 — "레드팀하기 훨씬 쉬운 모델"이 된 것이다. 아래 그림이 그 대비를 보여준다.
...
PM 학습에는 13M부터 52B까지, 약 4배씩 증가하는 7개 모델 크기를 썼다. 학습 전 [Askell et al., 2021]에서 제안한 preference model pretraining(PMP)을 거치고, 사람 선호 데이터로는 딱 1 epoch만 파인튜닝한다. 1 epoch만 돌리기 때문에 학습 곡선 자체가 곧 "데이터 양에 따른 스케일링"을 보여준다.
...
| 논문 이전 prompted 모델들 [Askell et al., 2021] | PM보다 낮음     |
...
저자들은 이 결과에서 중요한 함의를 하나 짚는다. **작은 모델만 가지고 정렬 연구를 하면, 큰 모델에 그대로 외삽했을 때 틀린 결론에 도달할 수 있다.** "정렬은 성능을 깎는다"는 통념이 애초에 작은 모델 실험에서 나온 것일 수 있다는 경고다.
...
3. **(현재 글)** HH-RLHF (Bai 2022) — helpful·harmless preference model
