...
**soft label**은 정답을 $$1.0$$이 아니라 $$0.9$$ 같은 **중간값으로 주는 것**이다. "이 쌍은 확실히 $$y_c$$가 낫다"가 아니라 "90% 정도 낫다"고 가르친다. label smoothing과 헷갈리기 쉬운데, **soft label은 목표값이고 label smoothing은 그 목표값을 만드는 방법**이다. 아래 식의 $$\alpha$$가 smoothing 강도이고 목표 확률이 $$1-\alpha$$가 되니, 결국 $$t = 1-\alpha$$인 soft label을 주는 셈이다.

왜 이게 필요한지는 hard label의 문제를 보면 드러난다. BT 손실 $$-\log\sigma(\Delta)$$($$\Delta = r_\psi(x,y_c) - r_\psi(x,y_r)$$)는 목표가 $$1.0$$일 때 **$$\Delta \to \infty$$에서만 최소**가 된다. 즉 점수 차이를 아무리 벌려도 **항상 더 벌리라는 압력**이 남는다. 도달할 목표점이 없는 것이다.

목표를 $$t$$로 낮추면 손실은 $$-[t\log\sigma(\Delta) + (1-t)\log(1-\sigma(\Delta))]$$가 되고, 이때 기울기는 $$\sigma(\Delta) - t$$다. 여기서 결정적인 차이가 생긴다.

| 현재 상태               | hard label ($$t=1$$) | soft label ($$t=0.9$$)     |
| ----------------------- | -------------------- | -------------------------- |
| $$\sigma(\Delta)=0.7$$  | 더 벌려라            | 더 벌려라                  |
| $$\sigma(\Delta)=0.9$$  | 더 벌려라            | **멈춰라** (기울기 0)      |
| $$\sigma(\Delta)=0.99$$ | 더 벌려라            | **되돌려라** (기울기 음수) |

최적 지점이 유한한 값으로 고정된다. $$t=0.9$$면 $$\Delta^\ast = \log(0.9/0.1) = \log 9 \approx 2.20$$이다. **너무 벌리면 오히려 끌어당기는 복원력이 생긴다**는 것이 핵심이다.

논문이 이 처방을 **강한 선호 그룹**에 쓰는 이유도 여기서 나온다. 그 그룹은 $$\sigma(\Delta)$$가 금방 1 근처로 가서 "더 벌려라" 압력만 남는 구간인데, 모델은 그 압력을 **가장 싸게** 해소할 방법 — 길이나 형식 같은 표면 패턴 — 을 찾아낸다. soft label이 그 폭주를 유한한 지점에서 끊는다.

**label smoothing**은 애매한 쌍에서 모델이 100% 확신을 갖지 않도록 목표 확률을 완화한다. 같은 원리를 반대쪽 극단에 적용하는 것이다.
...
측정된 선호 강도 $$\hat\mu(x,y)$$를 시그모이드 안쪽에서 빼주는 형태다. 그냥 두면 이미 $$\hat\mu_i$$가 큰 쌍은 손실이 금방 0 근처로 떨어져 그래디언트가 사라지는데, margin을 강도만큼 요구하면 "이미 벌어진 만큼 더 벌려야 손실이 0이 된다"는 조건이 걸려서 모델이 표면적인 패턴(길이, 형식 등)에 안주하지 않고 계속 판별력을 벼리게 된다.

soft label과 adaptive margin은 같은 문제를 정반대 방향에서 잡는다는 점이 재미있다. **soft label은 "목표를 낮춰" 멈추게 하고, margin은 "요구를 높여" 안주하지 못하게 한다.** 강한 선호 그룹에 둘을 함께 적용하는 이유가 여기 있다 — 과하게 벌리는 것도 막고, 쉽게 만족하는 것도 막는다. 이 margin 아이디어는 8편에서 다룰 [Llama 2](/blog/2026/llama2-rlhf/)의 RM 학습에서 프로덕션 버전으로 단순화되어 실제로 쓰인다.
...
세 쌍의 $$\hat\sigma_i$$가 0.21~0.24로 거의 같다는 점에 주목하자. **RM들의 의견 일치도는 셋 다 비슷한데, $$\hat\mu_i$$의 위치만으로 처방이 완전히 갈린다.** C는 세 RM이 입을 모아 "라벨이 거꾸로다"라고 말하는 경우라 폐기가 아니라 뒤집기 대상이고(뒤집으면 점수차 $$+1.5$$의 정상 쌍이 된다), B는 판단 자체가 서지 않는 쌍이며, A는 이미 충분히 벌어져 더 짜낼 신호가 없는 쌍이다.

## contrastive learning으로 표현을 단단히 하기

### 왜 표현을 따로 손봐야 하나

앞의 처방들은 전부 **손실에 들어가는 라벨**을 손봤다. flip은 방향을 고치고, smoothing과 margin은 목표값을 조절한다. 그런데 이걸 아무리 잘해도 건드리지 못하는 층이 하나 남는다. **RM이 응답을 어떤 벡터로 표현하는가**다.

BT 손실은 $$r_\psi(x,y_c) > r_\psi(x,y_r)$$만 요구한다. 이 조건을 만족시키는 가장 싼 방법은 무엇일까. **응답을 실제로 이해하는 것이 아니라, 정답과 상관된 표면 신호를 찾는 것**이다. 길이가 길면 대체로 chosen이더라, 목록 형식이면 chosen이더라 — 이런 지름길이 학습 데이터에서 통하면 RM은 거기 안주한다. 그러면 은닉 표현 $$h$$ 자체가 "길이·형식" 축으로만 조직되고, 내용은 비슷한데 길이만 같은 두 응답은 표현 공간에서 거의 겹쳐버린다. **구분할 수 없는 것을 구분하라고 요구해봐야 소용없다.**

그래서 논문은 라벨이 아니라 표현에 직접 손을 댄다. 목표는 하나다. **비슷해 보이는 응답들이 표현 공간에서 뭉개지지 않게 밀어내는 것.**

### 두 가지 방식

첫 번째는 **SimCSE 스타일**이다. 같은 입력을 dropout만 다르게 해서 두 번 통과시키면 약간 다른 두 벡터가 나오는데, 이 둘을 **양성 쌍**(같은 것이어야 함)으로 놓고 배치 안의 다른 응답들을 **음성 쌍**(달라야 함)으로 놓는다.

$$\ell_i = -\log \frac{\exp\big(\mathrm{sim}(h_s^{(i)}, h_t^{(i)})/\tau\big)}{\sum_{j=1}^{N'} \exp\big(\mathrm{sim}(h_s^{(i)}, h_t^{(j)})/\tau\big)}$$

기호를 풀면 $$h_s^{(i)}, h_t^{(i)}$$는 같은 응답 $$i$$를 두 번 통과시켜 얻은 두 표현, $$\mathrm{sim}$$은 코사인 유사도, $$\tau$$는 온도(temperature)다. 분자는 "자기 자신의 두 버전이 얼마나 닮았나", 분모는 "배치 안 모든 응답과 얼마나 닮았나"다. 이 비율을 키우려면 **자기 짝과는 붙고 남들과는 떨어져야** 한다.

여기서 dropout이 하는 역할이 영리하다. 텍스트는 이미지처럼 자르거나 회전시킬 수 없어서 augmentation을 만들기 어려운데, dropout이라는 **이미 있는 노이즈**를 augmentation으로 재활용한다. 별도 장치 없이 양성 쌍을 공짜로 얻는 것이다.

두 번째는 **SwAV 스타일**이다. 표현을 직접 비교하는 대신 프로토타입 $$\{c_1, \ldots, c_K\}$$이라는 **기준점 $$K$$개**를 두고, "이 응답이 어느 프로토타입에 속하는가"라는 클러스터 할당을 매긴다. 그리고 서로 다른 뷰에서 나온 할당이 **일치하도록** 강제한다.

|               | SimCSE 방식                 | SwAV 방식                    |
| ------------- | --------------------------- | ---------------------------- |
| 무엇을 맞추나 | 표현 벡터끼리 직접 비교     | 프로토타입 할당이 일치하는지 |
| 음성 쌍       | 배치 안 다른 응답 전부 필요 | 프로토타입이 대신함          |
| 성격          | 개별 비교                   | 군집 수준 비교               |

둘 다 원래 RM 손실에 가중치 $$\beta$$로 더해진다.
...
|               | 데이터 처방(flip/smoothing/margin) | contrastive 항                     |
| ------------- | ---------------------------------- | ---------------------------------- |
| 건드리는 것   | **라벨** — 어떤 쌍을 얼마나 믿을지 | **표현** — 응답을 어떤 벡터로 볼지 |
| 해결하는 문제 | 잘못되거나 애매한 신호             | 애초에 구분이 안 되는 표현         |
| 비유          | 채점 기준표를 고친다               | 채점자의 시력을 교정한다           |

기준표가 아무리 정확해도 답안지가 흐릿하게 보이면 소용없다. 반대로 시력이 좋아도 기준표가 틀렸으면 엉뚱하게 채점한다. 그래서 논문은 둘을 함께 쓴다.

이 구분이 특히 중요한 이유는 **OOD 상황**이다. RLHF가 진행되면 policy가 학습 데이터에 없던 응답을 만들어내는데, 이때 라벨 처방은 아무 도움이 안 된다. 그 응답에는 애초에 라벨이 없기 때문이다. 반면 표현이 잘 조직되어 있으면 처음 보는 응답도 제대로 배치되어 점수가 나온다. 논문이 PPO 학습 곡선에서 contrastive RM이 더 안정적으로 수렴한다고 보고하는 것도 이 맥락이고, 바로 다음 절의 MetaRM이 같은 OOD 문제를 또 다른 각도에서 공략한다.
...
<ol start="4">
  ...
  <li><strong>(현재 글)</strong> Secrets of RLHF II (2024) — 선호 데이터 노이즈와 RM 일반화</li>
