...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| MMLU (test, n=14,042)                 | 미달 — 표층 단서로 정답을 맞히는 사례가 보고됨, [#5](/blog/2026/clever-hans-benchmarks/)                                                                                                                                                                                                                                                                                                                                                                                       | 미달 — MMLU-Redux가 57개 전 과목 5,700문항을 전문가 14명이 재검수해 **9% 이상**에서 오류를 찾았고, 이를 층화표집으로 외삽한 **전체 MMLU 오류율 추정치는 6.49%**다. 과목 편차가 커서 Virology는 분석 문항의 **57%**에 문제가 있었다                                                            | 조건부 통과 — 전체 문항을 pooled micro-accuracy로 비교한다는 가정에서 1\~2%p 수준까지 탐지 가능(아래 계산)                                       | 미달 — 과목별 정확도만 보고, 인구집단·문화권별 성능 격차는 설계에 없음                                                     |
...
| KMMLU (test, n=35,030)                | 미달 — 원 시험 문항을 그대로 가져와 "한국어 이해"보다 "특정 자격시험 암기"를 재는 문항이 섞여 있음, [#14](/blog/2026/korean-benchmarks/)                                                                                                                                                                                                                                                                                                                                       | 미달 — 후속 KMMLU-Redux 작업이 원본에서 오류 문항 **7.66%**를 식별했다(GPT-4o 자동 주석이라, 사람 전문가가 검수한 MMLU-Redux와 같은 층위로 비교할 수는 없다). 이와 **별개로** 소형 LLM 7개 중 4개 이상이 맞힌 변별력 낮은 문항 **38.6%**를 걸러냈다 — 이쪽은 주석 오류가 아니라 난이도 문제다 | 조건부 통과 — 같은 micro 가정에서 1%p 초반까지 탐지 가능. 과목 macro-average가 최종 지표라면 별도 계산이 필요하다(아래 계산)                     | 미달 — 문항의 약 5분의 1이 한국 문화 지식을 요구한다고 원 논문이 밝혔지만, 지역·세대별 성능 격차를 분리해 재는 설계는 없음 |
...
HumanEval에는 단서가 하나 더 붙는다. pass@$$k$$는 문항당 여러 개의 코드를 생성해 추정하므로 변동의 층이 **문항 표집**과 **생성 표집** 둘이다. 위의 15%p는 각 문항의 통과 확률이 충분한 생성 표본으로 안정적으로 추정됐다고 보고 **문항 표집 오차만 센** 근사다.
...
2021년의 NLU 벤치마크에서는 이 축이 상대적으로 약했다. 문항은 고정돼 있었고 정답은 데이터셋에 들어 있었으며, 리더보드는 제출된 예측을 채점할 뿐이었다. 오늘의 평가는 그렇지 않다. **네 기준은 여전히 필요하지만, 충분하지 않다** — 여기에 평가 거버넌스라는 다섯 번째 축이 붙어야 한다는 것이, 이 채점표를 채워보고 얻는 부수적 소득이다.
