...
description: "RL Reward 설계 시리즈 #26 — prospect theory로 본 정렬, 이진 신호만으로 DPO를 따라잡다"
categories: [paper]
...
이 시리즈에서 지금까지 다룬 정렬 방법은 거의 전부 같은 전제 위에 서 있었다. **같은 프롬프트에 대한 두 응답 중 어느 쪽이 더 나은가**를 사람이 판단해야 한다는 전제다. RLHF의 reward model도, [#24 DPO](/blog/2026/dpo/)도, [#25 SimPO](/blog/2026/simpo/)도 전부 선호 쌍(preference pair) $$(y_w, y_l)$$을 원재료로 삼는다.
...
이 시리즈의 5부("reward를 정책으로")는 [#24 DPO](/blog/2026/dpo/)를 시작으로 RL 없이 정책을 직접 최적화하는 흐름을 다뤄왔다. [#25 SimPO](/blog/2026/simpo/)가 reference 정책을 없애는 방향으로 DPO를 단순화했다면, KTO는 다른 축을 건드린다. 손실 함수의 형태가 아니라 **입력 데이터의 형태** 자체를 바꾸는 쪽이다.
...
시리즈 관점에서 보면 KTO는 [#24 DPO](/blog/2026/dpo/)와 [#25 SimPO](/blog/2026/simpo/)가 "손실 함수를 어떻게 바꿀 것인가"에 집중했던 것과 결이 다르다. 손실 함수의 이론적 근거(prospect theory)를 새로 들여오면서, 동시에 "애초에 어떤 데이터로 학습할 것인가"라는 질문 자체를 다시 던진 것에 가깝다. 선호 쌍 수집이라는 병목을 손실 함수 설계로 우회할 수 없다면, 데이터 형식을 바꾸는 것도 하나의 답이 될 수 있다는 사례로 이 시리즈에 남는다.
...
이 글은 RL Reward 설계 시리즈의 스물여섯 번째 글이다.
...
<ol start="20">
  ...
  <li><strong>(현재 글)</strong> KTO (2024) — 선호 쌍 없이 이진 신호만으로</li>
