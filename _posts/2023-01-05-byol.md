...
> [Bootstrap Your Own Latent: A New Approach to Self-Supervised Learning](https://arxiv.org/abs/2006.07733) (Grill et al., DeepMind, NeurIPS 2020)
...
위는 contrastive learning의 loss function인 InfoNCE이다.
...
SimCLR은 batch에 positive sample과 negative sample을 함께 구성하여 negative sample의 수를 확보한다.
...
이러한 특성으로 batch size가 작아도 많은 negative sample을 확보할 수 있다.
...
Encoder를 random initialization한 후 parameter를 고정한다. 그리고 linear layer를 하나 붙여 ImageNet에 학습을 시킬 경우 정확도가 1.8%로 낮았다.
...
4. Online network의 parameter는 backpropagation을 통해 update한다.
