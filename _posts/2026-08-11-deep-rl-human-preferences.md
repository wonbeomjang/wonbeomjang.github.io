...
에이전트는 매 스텝 관측 $$o_t$$를 받고 행동 $$a_t$$를 내보낸다. 전통적 RL이라면 환경이 보상 $$r_t$$를 주지만, 여기서는 그 대신 사람 오버시어가 **궤적 세그먼트(trajectory segment)** 쌍에 대해 선호를 표현한다. 세그먼트는 길이 $$k$$의 관측·행동 시퀀스다.

$$\sigma = ((o_0, a_0), (o_1, a_1), \ldots, (o_{k-1}, a_{k-1}))$$

$$\sigma^1 \succ \sigma^2$$는 "사람이 $$\sigma^1$$을 $$\sigma^2$$보다 선호했다"는 뜻이다. 저자들은 알고리즘의 성능을 두 방식으로 평가한다.

- **Quantitative(정량)**: 실제 보상 함수 $$r$$이 존재하는 벤치마크 태스크에서, 알고리즘이 그 $$r$$을 몰라도 $$r$$을 아는 것처럼 학습하는지 본다.
- **Qualitative(정성)**: 백플립처럼 애초에 보상 함수가 없는 태스크에서, 자연어로 목표를 던지고 사람이 결과 영상을 보고 만족스러운지 판단한다.

# Method

## 세 개의 프로세스가 비동기로 돈다

이 방법은 정책 $$\pi: O \to A$$와 보상 함수 추정치 $$\hat r: O \times A \to \mathbb{R}$$을 동시에 유지한다. 둘 다 심층신경망이다. 세 프로세스가 서로 다른 속도로 병렬 실행된다.

| 프로세스           | 하는 일                                                                                                                             | 담당 알고리즘                           |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| (1) 정책 롤아웃    | $$\pi$$가 환경과 상호작용해 궤적 $$\{\tau^1, \ldots, \tau^i\}$$ 생성. $$\pi$$는 $$r_t = \hat r(o_t, a_t)$$의 합을 최대화하도록 갱신 | Atari는 A2C, MuJoCo는 TRPO              |
| (2) 라벨 수집      | (1)에서 나온 궤적들 중 세그먼트 쌍 $$(\sigma^1, \sigma^2)$$을 골라 사람에게 질의                                                    | 앙상블 불일치 기반 uncertainty sampling |
| (3) 보상 모델 학습 | 지금까지 쌓인 비교 데이터베이스 $$D$$로 $$\hat r$$을 지도학습으로 갱신                                                              | Bradley-Terry cross-entropy             |

흐름은 (1)→(2)로 궤적이, (2)→(3)으로 사람 라벨이, (3)→(1)로 갱신된 $$\hat r$$ 파라미터가 흐른다. 세 프로세스가 동시에 돈다는 것이 핵심이다 — 정책이 계속 새로운 상태를 방문하면, 보상 모델도 그 새 상태에 대한 라벨을 계속 받아 따라간다.
...
이 글은 RLHF Reward 설계 시리즈의 첫 번째 글이다.

**1부. 지형도**

1. **(현재 글)** Deep RL from Human Preferences (Christiano 2017) — 선호로 보상을 배우는 원형
