...
description: "RLHF Reward 설계 시리즈 #29 — KL 제약과 reward 최적화의 줄다리기를 weight averaging으로 푼다"
categories: [paper]
...
이 세 단계를 반복하면서 KL-reward 파레토 프런티어 자체를 점진적으로 밀어올린다. 흥미로운 대목은 이 논문이 [#13 WARM](/blog/2026/warm-weight-averaged-reward/)에 대한 응답으로 쓰였다는 점이다. WARM이 여러 reward model을 평균해 reward 설계 쪽에 weight averaging을 적용했다면, WARP는 같은 아이디어를 정책 학습 쪽에 적용한다.

# Background

## KL 정규화의 원형

[#19 PPO](/blog/2026/ppo/)의 RLHF objective는 다음과 같은 형태다.
...
[#13 WARM](/blog/2026/warm-weight-averaged-reward/)이 reward 설계 쪽에서, WARP가 정책 학습 쪽에서 같은 weight averaging 아이디어를 각자의 방식으로 적용했듯, [#28 BOND](/blog/2026/bond/)를 포함해 이 시기 DeepMind의 정렬 연구들은 서로 다른 컴포넌트에서 비슷한 직관을 공유한다. 실제로 [#43 프론티어 모델의 reward 설계](/blog/2026/frontier-reward-design/)에서 다루듯, Gemma의 post-training 레시피는 BOND, WARM, WARP를 함께 조합한 결과물이다.
