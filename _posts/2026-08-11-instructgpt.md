...
SFT는 약 13k, RM은 약 33k, PPO는 약 31k 프롬프트로 학습한다. 흥미로운 점은 SFT 단계는 라벨러가 직접 쓴 프롬프트가 압도적으로 많고(11,295 대 1,430), RM·PPO로 갈수록 실제 API 트래픽(customer) 비중이 커진다는 것이다. 초기에는 시범을 보여줄 "정답"이 필요했으니 라벨러가 직접 프롬프트와 답을 함께 만들었고, 이후에는 실제 배포 트래픽에서 뽑은 프롬프트에 대해 "어느 응답이 더 나은가"만 순위 매기면 되기 때문에 API 프롬프트를 그대로 쓸 수 있었던 것이다.
...
해법은 그 프롬프트에서 나온 $$\binom{K}{2}$$개의 쌍 **전체를 하나의 배치 원소로 묶어서** 학습하는 것이다. 이렇게 하면 두 가지가 동시에 해결된다.

- **연산 효율**: $$K$$개 응답에 대해 forward pass를 단 한 번만 하면 되고(각 완성문을 한 번씩만 인코딩), 그 결과로 $$\binom{K}{2}$$개 쌍의 손실을 모두 계산한다. $$\binom{K}{2}$$번 forward pass를 도는 것보다 훨씬 싸다.
- **overfitting 해소**: 같은 프롬프트의 상관된 비교들이 한 번의 gradient step 안에서 함께 처리되므로, 반복 노출로 인한 과최적화가 사라진다. 그 결과 validation accuracy와 log loss가 크게 개선됐다.
...
순진한 방식이면 이 3개 쌍을 서로 다른 배치(혹은 다른 스텝)에 흩어 넣는다. 이 경우 응답 B는 두 번, A는 두 번(승자 한 번, 패자 한 번) gradient에 등장해 총 4번의 업데이트에 관여한다. InstructGPT의 방식은 A, B, C를 **한 번씩만 인코딩**해 $$r_\theta(x,A)$$, $$r_\theta(x,B)$$, $$r_\theta(x,C)$$ 세 스칼라를 얻고, 이 세 값으로 위 3개 쌍의 손실을 동시에 계산해 **하나의 배치 원소**로 묶어 한 번에 역전파한다. 인코딩은 3번, 손실 항은 3개 — forward pass 횟수가 순진한 방식의 절반 이하로 줄고, 같은 데이터가 여러 스텝에 걸쳐 반복 노출되지도 않는다.
...
실제 값은 $$\beta = 0.02$$, $$\gamma = 27.8$$이다. 흥미로운 실험 결과는 **KL 계수 $$\beta$$만 키우는 것으로는 이 회귀를 되돌릴 수 없었다**는 점이다 — 기본값의 100배인 $$\beta=2.0$$까지 올려도 SQuAD·DROP 성능 저하는 완전히 회복되지 않았고, 오히려 validation reward만 크게 깎였다. 반면 $$\gamma \geq 20$$(1.3B 모델 기준) 수준으로 사전학습 항을 섞으면 회귀가 상당 부분 복구됐다. 이는 "SFT에서 안 멀어지게" 막는 것과 "사전학습 능력을 안 잃게" 지키는 것이 서로 다른 문제이며, 후자를 고치려면 후자에 해당하는 신호(사전학습 loss)를 직접 넣어야 한다는 걸 보여준다.
