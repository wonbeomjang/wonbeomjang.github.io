...
description: "RL Reward 설계 시리즈 #24 — 암묵적 reward로 RL을 우회한 대가, 그리고 다시 명시적 RM으로 돌아온 이유"
categories: [paper]
...
20편부터 23편까지, 이 시리즈의 5부는 전부 같은 전제 위에 서 있었다. "reward model $$r_\phi$$는 이미 있다. 그걸로 정책을 어떻게 최적화할 것인가?" PPO([#20](/blog/2026/ppo/))는 clipped surrogate로, Secrets of RLHF I([#21](/blog/2026/secrets-rlhf-ppo/))는 그 PPO를 안정시키는 트릭으로, GRPO([#22](/blog/2026/grpo-deepseekmath/))는 value network를 버리는 방식으로, RLOO([#23](/blog/2026/rloo-back-to-basics/))는 아예 REINFORCE로 되돌아가는 방식으로 이 질문에 답했다. 네 편 모두 "정책 최적화 알고리즘"을 다퉜을 뿐, reward model이라는 부품 자체는 건드리지 않았다.
...
이 글은 단순히 "DPO가 무엇인가"를 소개하지 않는다. **reward model을 없애서 얻은 것과 잃은 것을 항목별로 대차대조표처럼 정리**하는 것이 목표다. 얻은 쪽은 짧다 — 안정성과 단순함. 잃은 쪽은 길고, 그 길이가 곧 뒤에 이어질 6부(Process & Verifiable Reward)와 7부(Generative Reward Model)가 왜 다시 명시적인, 심지어 더 촘촘한 reward로 돌아가는지를 설명해준다.
...
RLHF의 최종 목적함수는 20편에서 다룬 것과 동일하다.
...
| Value/Critic 네트워크       | PPO는 필요 (16, 23편에서 제거 시도)           | 없음                                                            |
...
- **RM 학습 단계 제거**: 별도 reward model을 위한 pairwise 데이터 학습 라운드, 체크포인트 관리, calibration 검증([#9 RewardBench 2류의 평가](/blog/2026/rewardbench-2/))이 통째로 사라진다.
- **RL 루프 제거**: on-policy rollout, PPO clip, GAE, value network(16, 23편에서 이미 문제 삼은 것들)에서 자유로워진다.
...
**(d) 선호 데이터 품질에 그대로 종속.** RM을 따로 학습하는 파이프라인에서는 노이즈 필터링, 앙상블([#14 WARM](/blog/2026/warm-weight-averaged-reward/)), 다목적 분해([#7 ArmoRM](/blog/2026/armorm/)) 같은 완충 장치를 RM 학습 단계에 끼워 넣을 수 있었다. DPO는 그 단계 자체가 없으므로, 선호 라벨의 노이즈와 편향이 필터링 없이 그래디언트에 곧바로 반영된다.
...
23편에서 다룬 RLOO는 여러 정렬 벤치마크에서 DPO보다 나은 성능을 보고하며, online RL이 offline DPO보다 유리한 지점이 있다고 주장했다. 근거는 정확히 위 (a)와 궤를 같이한다 — RLOO는 매 스텝 정책에서 직접 샘플링하고 그 샘플에 reward model이 점수를 매기므로, 정책이 이동한 지점에서의 reward를 계속 관측할 수 있다.

7부(34~38편)의 GenRM(Generative Reward Model) 부흥도 같은 공백에서 출발한다. DPO가 남긴 두 가지 공백 — 명시적으로 재사용 가능한 reward 산출물의 부재(b), 그리고 그 reward를 process 수준으로 세분화할 수 없다는 한계 — 를 정면으로 채우는 것이 GenRM이다. reward를 다음 토큰 예측으로 재정의하고([#34](/blog/2026/prometheus-2/)), inference-time scaling에 재사용 가능한 채점기로 되돌리는 흐름이 여기서 시작된다.
...
이제 5부(20~24편) 전체를 한 줄로 요약할 수 있다. PPO([#20](/blog/2026/ppo/))로 시작해 그것을 안정화하고([#21](/blog/2026/secrets-rlhf-ppo/)), 단순화하고([#22](/blog/2026/grpo-deepseekmath/)), 최소화하고([#23](/blog/2026/rloo-back-to-basics/)), 마침내 제거해본(24편) 다섯 편은 결국 "reward를 소비하는 알고리즘을 어디까지 가볍게 만들 수 있는가"에 대한 실험이었다. 다음 6부는 정반대 방향으로 움직인다. reward를 없애는 대신, **응답 전체가 아니라 한 단계 한 단계에** reward를 매기는 방향으로 촘촘하게 만든다. 31편 [Let's Verify Step by Step](/blog/2026/lets-verify-step-by-step/)에서 이어진다.
...
이 글은 RL Reward 설계 시리즈의 스물네 번째 글이다.
...
<ol start="20">
  ...
  <li><strong>(현재 글)</strong> DPO (2023) — reward를 없애면 어떻게 되는가</li>
