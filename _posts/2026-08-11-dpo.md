...
description: "RLHF Reward 설계 시리즈 #23 — 암묵적 reward로 RL을 우회한 대가, 그리고 다시 명시적 RM으로 돌아온 이유"
categories: [paper]
...
19편부터 22편까지, 이 시리즈의 5부는 전부 같은 전제 위에 서 있었다. "reward model $$r_\phi$$는 이미 있다. 그걸로 정책을 어떻게 최적화할 것인가?" PPO([19편](/blog/2026/ppo/))는 clipped surrogate로, Secrets of RLHF I([20편](/blog/2026/secrets-rlhf-ppo/))는 그 PPO를 안정시키는 트릭으로, GRPO([21편](/blog/2026/grpo-deepseekmath/))는 value network를 버리는 방식으로, RLOO([22편](/blog/2026/rloo-back-to-basics/))는 아예 REINFORCE로 되돌아가는 방식으로 이 질문에 답했다. 네 편 모두 "정책 최적화 알고리즘"을 다퉜을 뿐, reward model이라는 부품 자체는 건드리지 않았다.
...
RLHF의 최종 목적함수는 19편에서 다룬 것과 동일하다.
...
| Value/Critic 네트워크       | PPO는 필요 (16, 22편에서 제거 시도)           | 없음                                                            |
...
- **RL 루프 제거**: on-policy rollout, PPO clip, GAE, value network(16, 22편에서 이미 문제 삼은 것들)에서 자유로워진다.
...
22편에서 다룬 RLOO는 여러 정렬 벤치마크에서 DPO보다 나은 성능을 보고하며, online RL이 offline DPO보다 유리한 지점이 있다고 주장했다. 근거는 정확히 위 (a)와 궤를 같이한다 — RLOO는 매 스텝 정책에서 직접 샘플링하고 그 샘플에 reward model이 점수를 매기므로, 정책이 이동한 지점에서의 reward를 계속 관측할 수 있다.

7부(33~37편)의 GenRM(Generative Reward Model) 부흥도 같은 공백에서 출발한다. DPO가 남긴 두 가지 공백 — 명시적으로 재사용 가능한 reward 산출물의 부재(b), 그리고 그 reward를 process 수준으로 세분화할 수 없다는 한계 — 를 정면으로 채우는 것이 GenRM이다. reward를 다음 토큰 예측으로 재정의하고([33편](/blog/2026/prometheus-2/)), inference-time scaling에 재사용 가능한 채점기로 되돌리는 흐름이 여기서 시작된다.
...
이제 5부(19~23편) 전체를 한 줄로 요약할 수 있다. PPO([19편](/blog/2026/ppo/))로 시작해 그것을 안정화하고([20편](/blog/2026/secrets-rlhf-ppo/)), 단순화하고([21편](/blog/2026/grpo-deepseekmath/)), 최소화하고([22편](/blog/2026/rloo-back-to-basics/)), 마침내 제거해본(23편) 다섯 편은 결국 "reward를 소비하는 알고리즘을 어디까지 가볍게 만들 수 있는가"에 대한 실험이었다. 다음 6부는 정반대 방향으로 움직인다. reward를 없애는 대신, **응답 전체가 아니라 한 단계 한 단계에** reward를 매기는 방향으로 촘촘하게 만든다. 30편 [Let's Verify Step by Step](/blog/2026/lets-verify-step-by-step/)에서 이어진다.
