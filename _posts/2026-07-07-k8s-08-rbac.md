...
```mermaid
flowchart LR
    C["kubectl / Pod"] -->|"TLS 요청<br>(기본 6443 포트)"| A["1단계: 인증<br>Authentication"]
    A -->|"신원 확인됨"| Z["2단계: 인가<br>Authorization<br>(RBAC)"]
    Z -->|"허용된 요청만"| AD["3단계: 어드미션<br>Admission Control"]
    AD --> E["검증 후 etcd에 저장"]
    A -.->|"실패: 401"| X["요청 거부"]
    Z -.->|"거부: 403"| X
```
...
```mermaid
flowchart LR
    SA["ServiceAccount<br>pod-viewer<br>(누구)"] -->|"subjects로 지정"| RB["RoleBinding<br>read-pods<br>(연결 고리)"]
    RB -->|"roleRef로 참조"| R["Role<br>pod-reader<br>(무엇을 할 수 있나)"]
    R -->|"get / list / watch 허용"| RES["pods 리소스<br>(rbac-lab 네임스페이스)"]
```
