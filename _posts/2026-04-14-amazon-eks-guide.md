...
{% include figure.liquid loading="lazy" path="assets/post/image/amazon-eks-guide/eks-architecture.png" class="img-fluid rounded z-depth-1" alt="EKS 아키텍처: AWS 관리 Control Plane과 사용자 VPC" %}
...
Karpenter는 AWS가 만든 **오픈소스 노드 오토스케일러**로, Cluster Autoscaler를 대체한다. Pod이 Pending 상태가 되면 **워크로드에 맞는 최적의 인스턴스를 직접 선택**하여 수 초 내에 노드를 프로비저닝한다.
...
1. **GPU 인스턴스 자동 선택**: Pod이 `nvidia.com/gpu: 4`를 요청하면, Karpenter가 p4d.24xlarge, g5.12xlarge 등 사용 가능한 GPU 인스턴스 중 **가장 저렴한 것**을 자동으로 선택한다
...
EKS Auto Mode는 **Karpenter가 Control Plane에 내장된 것**이다. 직접 Karpenter를 설치·관리할 필요 없이 동일한 기능을 쓸 수 있다. 다만 Auto Mode는 커스터마이징이 제한적이므로, **NodePool을 세밀하게 제어**하고 싶다면 Karpenter를 직접 설치하는 것이 낫다.
...
{% include figure.liquid loading="lazy" path="assets/post/image/amazon-eks-guide/vpc-cni.png" class="img-fluid rounded z-depth-1" alt="VPC CNI: Pod에 VPC IP 직접 할당 구조" %}
...
{% include figure.liquid loading="lazy" path="assets/post/image/amazon-eks-guide/pod-identity.png" class="img-fluid rounded z-depth-1" alt="EKS Pod Identity: Pod에서 AWS 서비스 접근 흐름" %}
...
{% include figure.liquid loading="lazy" path="assets/post/image/amazon-eks-guide/cost-formula.png" class="img-fluid rounded z-depth-1" alt="EKS 비용 최적화 공식" %}
...
{% include figure.liquid loading="lazy" path="assets/post/image/amazon-eks-guide/upgrade-flow.png" class="img-fluid rounded z-depth-1" alt="EKS 클러스터 업그레이드 순서" %}
...
{% include figure.liquid loading="lazy" path="assets/post/image/amazon-eks-guide/deploy-flow.png" class="img-fluid rounded z-depth-1" alt="EKS 배포 파이프라인 흐름" %}
