...
```mermaid
flowchart LR
    A["<b>Borg</b><br/>구글 내부 클러스터 매니저<br/>10년 이상 내부 운영<br/>논문 공개: EuroSys 2015"] --> B["<b>Omega</b><br/>차세대 스케줄러 연구<br/>공유 상태 + 낙관적 동시성 제어<br/>논문: EuroSys 2013"]
    B --> C["<b>Kubernetes</b><br/>오픈소스 공개: 2014년 6월<br/>v1.0: 2015년 7월 21일"]
    C --> D["<b>CNCF</b><br/>2015년 7월 21일 창립<br/>Linux Foundation 산하<br/>K8s가 시드 프로젝트"]
```
...
```mermaid
flowchart TD
    A["사용자: 원하는 상태 선언<br/>예: nginx 컨테이너 3개"] --> B["Kubernetes: 현재 상태 관찰"]
    B --> C{"원하는 상태와<br/>일치하는가?"}
    C -- "일치한다" --> B
    C -- "다르다<br/>예: 1개 죽어서 2개" --> D["차이를 메우는 조치<br/>예: 컨테이너 1개 새로 생성"]
    D --> B
```
