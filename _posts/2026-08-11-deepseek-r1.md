...
description: "RLHF Reward 설계 시리즈 #30 — RLVR이 통하는 도메인과 통하지 않는 도메인, 그리고 PRM을 버린 이유"
categories: [paper]
...
29편까지 이 시리즈가 다룬 질문은 한결같이 "reward model을 어떻게 더 잘 만드는가"였다. 사람 선호를 어떻게 스칼라로 압축할지([#4](/blog/2026/bradley-terry-rethinking/) [#5](/blog/2026/secrets-rlhf-reward-modeling/)), 데이터를 어떻게 큐레이션할지([#6](/blog/2026/skywork-reward/)), reward model이 hacking당하는 걸 어떻게 막을지([#10](/blog/2026/reward-model-overoptimization/)~[#12](/blog/2026/odin-disentangled-reward/)), 단계별로 감독하는 PRM을 사람 없이 어떻게 만들지([#29](/blog/2026/math-shepherd/)). 방향은 늘 같았다 — reward model을 더 정교하게, 더 견고하게.
...
4. 저자들이 왜 PRM과 MCTS를 시도했다가 접었는가 — [#28](/blog/2026/lets-verify-step-by-step/), [#29](/blog/2026/math-shepherd/)과의 정면 대비
...
[#28](/blog/2026/lets-verify-step-by-step/)와 [#29 Math-Shepherd](/blog/2026/math-shepherd/)는 결과만 보는 ORM보다 각 추론 단계를 감독하는 PRM이 더 정밀한 신호를 준다고 결론지었다. DeepSeek-R1 팀도 PRM을 시도했다. 논문에 남긴 실패 이유를 그대로 옮기면 이렇다.
...
2. **중간 정확도 판정의 어려움**: [#29](/blog/2026/math-shepherd/)의 Math-Shepherd는 이 문제를 몬테카를로 rollout 완료율로 우회했다 — "이 단계에서 이어 풀었을 때 몇 번이나 정답에 도달하는가"로 단계 라벨을 추정한 것이다. 이 방식은 수학처럼 rollout을 많이 돌려도 싸고 정답 판정이 쉬운 도메인에서나 성립한다. 일반 추론에서는 rollout 자체의 정답 판정이 이미 애매해서 라벨이 노이즈투성이가 된다.
...
4. **PRM과 MCTS를 접은 이유**: 단계 정의의 모호함, 중간 정확도 판정의 어려움, 그리고 PRM 자체가 다시 hacking 대상이 된다는 점. [#28](/blog/2026/lets-verify-step-by-step/), [#29](/blog/2026/math-shepherd/)이 공들여 만든 과정 감독을, R1은 "검증 가능한 도메인이면 아예 필요 없다"고 잘라냈다.
