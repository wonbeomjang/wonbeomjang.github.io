...
20편까지 이 시리즈가 다룬 질문은 한결같이 "reward model을 어떻게 더 잘 만드는가"였다. 사람 선호를 어떻게 스칼라로 압축할지([#4](/blog/2026/bradley-terry-rethinking/) [#5](/blog/2026/secrets-rlhf-reward-modeling/)), 데이터를 어떻게 큐레이션할지([#6](/blog/2026/skywork-reward/)), reward model이 hacking당하는 걸 어떻게 막을지([#10](/blog/2026/reward-model-overoptimization/)~[#13](/blog/2026/odin-disentangled-reward/)), 단계별로 감독하는 PRM을 사람 없이 어떻게 만들지([#20](/blog/2026/math-shepherd/)). 방향은 늘 같았다 — reward model을 더 정교하게, 더 견고하게.
...
1. **RLVR**: 학습된 RM 대신 규칙 기반 검증기(정답 일치, 테스트 통과)가 reward를 준다. 파라미터가 없는 함수이므로 3부([#10](/blog/2026/reward-model-overoptimization/)~[#13](/blog/2026/odin-disentangled-reward/))의 reward hacking이 정의상 성립하지 않는다.
