...
```python
# FA1: 매 스텝마다 정규화
O_new = diag(l_new)^{-1} * (alpha * diag(l_old) * O_old + exp(S - m_new) * V)

# FA2: un-scaled 누적
acc = acc * alpha[:, None]            # max 기준 보정만
acc = tl.dot(p.to(v.dtype), v, acc)  # 정규화 없이 누적

# ... 루프 종료 후
acc = acc / l_i[:, None]             # 마지막에 딱 1회
```
...
```python
# FA1: 매 exp마다 내부적으로 × log₂e 계산
p = tl.exp(qk * sm_scale - l_i[:, None])
alpha = tl.exp(m_i - m_ij)

# FA2: qk_scale에 log₂e 미리 포함 (constexpr → 런타임 비용 없음)
LOG2E: tl.constexpr = 1.4426950408889634
qk_scale = sm_scale * LOG2E

p = tl.math.exp2(qk * qk_scale - l_i_log2[:, None])
...
```python
# 커널 내부: base-2 LSE 저장
lse = m_i + tl.math.log2(l_i)  # base-2

# 래퍼 함수: 외부에는 자연로그로 노출
LN2 = 0.6931471805599453
lse_natural = lse_base2 * LN2
```
...
```python
@triton.jit
def _fa2_inner(acc, l_i, m_i, q, k_base, v_base, ..., STAGE: tl.constexpr):
    if STAGE == 1:
        lo, hi = 0, start_m * BLOCK_M          # 마스크 불필요 블록들
    elif STAGE == 2:
        lo = start_m * BLOCK_M
        hi = tl.minimum((start_m + 1) * BLOCK_M, seq_len)  # 대각선 블록
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        else:  # STAGE 1, 3: 마스크 분기 없음
            qk = qk * qk_scale + tl.where(offs_n[None, :] < seq_len, 0.0, -1.0e6)
```
...
```python
if IS_CAUSAL:
    # STAGE 1: 대부분의 연산 (마스크 오버헤드 없음)
    acc, l_i, m_i = _fa2_inner(..., STAGE=1)
    # STAGE 2: 대각선 블록 1개만 (마스크 적용)
...
```python
dv = tl.dot(tl.trans(p.to(do.dtype)), do, dv)   # dV += P^T @ dO
dk = tl.dot(tl.trans(ds.to(q.dtype)), q, dk)     # dK += dS^T @ Q
dq = tl.dot(ds.to(k.dtype), k, dq)               # dQ += dS @ K
```
...
```python
# FA1 backward: 자연로그 LSE
p = tl.exp(qk * sm_scale - l_i[:, None])

# FA2 backward: base-2 LSE (forward에서 저장한 형태 그대로)
LOG2E: tl.constexpr = 1.4426950408889634
qk_scale = sm_scale * LOG2E
p = tl.math.exp2(qk * qk_scale - l_i[:, None])  # l_i는 base-2 LSE
```
