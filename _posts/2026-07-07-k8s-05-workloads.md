...
```mermaid
flowchart TD
    U["kubectl set image<br/>nginx:1.25 → nginx:1.26"] --> D["Deployment nginx-deploy<br/>desired: nginx:1.26, replicas 5"]
    D -->|"점진적으로 0까지 축소"| OLD["구 ReplicaSet<br/>nginx-deploy-7c5d6b8f9d (1.25)"]
    D -->|"점진적으로 5까지 확장"| NEW["신 ReplicaSet<br/>nginx-deploy-6f9c4b7d5b (1.26)"]
    OLD -.->|Terminating| OP1["Pod (1.25)"]
    OLD -.->|Terminating| OP2["Pod (1.25)"]
    NEW --> NP1["Pod (1.26)"]
    NEW --> NP2["Pod (1.26)"]
    NEW --> NP3["Pod (1.26)"]
```
...
```mermaid
flowchart LR
    A["web-0 생성"] --> B["Running & Ready 확인"] --> C["web-1 생성"] --> D["Running & Ready 확인"] --> E["web-2 생성"]
```
