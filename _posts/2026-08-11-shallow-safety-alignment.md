...
이 형태는 낯설지 않다. log-ratio를 sigmoid에 넣고 음의 로그를 취하는 구조는 [#23 DPO](/blog/2026/dpo/)의 목적함수와 뼈대가 같다. 다른 점은 DPO가 (선호, 비선호) 응답 쌍 전체에 하나의 $$\beta$$를 쓰는 반면, 여기서는 같은 응답 안에서도 토큰 위치마다 $$\beta_t$$를 다르게 준다는 것이다. $$\beta_t$$가 작을수록 $$\pi_\theta$$가 $$\pi_{\text{aligned}}$$에서 벗어나는 데 대한 페널티가 가팔라진다. 즉 **작은 $$\beta_t$$ = 그 위치를 강하게 붙잡아 둔다**는 뜻이다.

논문이 실제로 쓴 설정은 $$\beta_1 = 0.5$$, $$t \in [2,5]$$ 구간에서 $$\beta_t = 2$$, $$t > 5$$에서는 $$\beta_t = 0.1$$이다. 얼핏 보면 뒤쪽(t > 5)의 $$\beta_t$$가 가장 작아서 가장 세게 묶이는 것처럼 보이지만, 논문의 설계 의도는 정확히 반대다. 앞쪽 5토큰 구간에는 상대적으로 느슨한 제약($$\beta_t = 0.5, 2$$)을 줘서 정상적인 task 적응을 방해하지 않게 하면서도, 정렬이 걸려 있던 그 좁은 창 자체는 별도로 계속 감시하고, 나머지 구간($$t > 5$$)은 강한 제약($$\beta_t = 0.1$$)으로 묶어 파인튜닝이 유해한 방향으로 응답 전체를 끌고 가지 못하게 막는다. 핵심은 "앞쪽 몇 토큰만 지키면 끝"이라는 shallow alignment의 약점을 정반대로 뒤집어서, 파인튜닝이 건드릴 수 있는 자유도 자체를 위치별로 통제하는 것이다.
