...
### 왜 A100 peak 대비 ~37%밖에 안 나오나

A100 80GB SXM4의 FP16 Tensor Core 이론 peak는 **312 TFLOP/s**다. Attention forward의 matmul FLOPs는 $$4 \cdot BH \cdot N^2 \cdot d$$ (S = QK^T와 O = PV 두 번의 matmul). 측정 시간으로 나누면 실제 throughput이 나온다.
...
긴 seq 점근선이 ~37%다. FlashAttention 공식 CUDA 구현은 같은 A100에서 ~70%를 찍는다 — 우리는 왜 절반밖에 안 되나?

**원인 분해**:

1. **매 블록마다 `acc /= l_i`** (Tc회 division)
   - N=8192, BLOCK_N=64면 Tc = 128. 매 K/V 블록 처리 후 BLOCK_M개 fp32 division이 들어가 matmul throughput을 끊는다.
   - GPU의 fp32 division은 reciprocal + multiply로 처리되고 SFU(special function unit) 가 별도라 Tensor Core dispatch와 충돌한다.
   - **추정 손실 10–15%**. FA2의 가장 큰 개선 항목이 이것.

2. **`tl.exp`의 숨은 곱셈**
   - `tl.exp(x)`는 컴파일러가 `exp2(x · log₂e)`로 변환한다. 매 iteration에 BLOCK_M × BLOCK_N개의 추가 fp32 multiply가 누적된다.
   - SFU에서 exp2는 1 cycle, mul은 ALU 1 cycle이라 dispatch가 두 갈래로 흩어진다.
   - **추정 손실 3–5%** (FA2의 `qk_scale * log₂e` pre-multiply로 회수).

3. **causal mask 매 iteration 분기**
   - `if IS_CAUSAL: ...`가 inner loop 안에 있어 컴파일러가 dead code elimination을 못 한다.
   - non-causal에서는 영향 없지만 causal일 때 매 iteration mask 계산이 일어난다.
   - **추정 손실 5–10%** (causal에 한해, FA2의 STAGE 1/2 분리로 회수).

4. **fp16 ↔ fp32 round-trip 캐스팅**
   - K/V는 fp16, `tl.dot` 결과는 fp32 누적. 매 iteration `p.to(v.dtype)` 캐스팅이 register pressure를 늘려 occupancy를 깎는다.
   - **추정 손실 2–3%**.

5. **Triton vs CUTLASS PTX 격차**
   - FlashAttention 공식은 hand-written CUDA로 wmma/mma instruction을 직접 배치한다.
   - Triton은 LLVM → PTX를 거치므로 register allocation·instruction scheduling에서 ~5–10% 손해가 일반적.
...
FA1에서 잃은 이론 peak의 약 30–40% 중 절반 이상은 FA2에서 회수한다 (다음 포스트 참고). 나머지 10–15%는 "Triton 추상화의 천장" — 더 짜내려면 CUTLASS 또는 Hopper 신명령어가 필요하다.
