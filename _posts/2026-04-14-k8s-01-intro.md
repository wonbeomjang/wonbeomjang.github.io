...
> - 05: Amazon EKS — 아키텍처와 Worker Node (예정)
...
컨테이너(Pod)가 **실제로 실행되는 서버**다. EC2 인스턴스라고 생각하면 된다.

| 구성 요소             | 역할                                                   | 비유        |
| --------------------- | ------------------------------------------------------ | ----------- |
| **kubelet**           | 각 노드에서 Pod을 실행하고 상태를 Control Plane에 보고 | 공장 관리자 |
| **kube-proxy**        | Pod 간 네트워크 통신과 로드밸런싱 처리                 | 우체부      |
| **Container Runtime** | 실제로 컨테이너를 실행하는 엔진 (containerd, CRI-O)    | 기계        |
...
4. **Scheduler**가 "Node 1에 2개, Node 2에 1개 배치" 결정
5. 각 노드의 **kubelet**이 Container Runtime을 통해 Pod 실행
6. Pod이 죽으면 Controller Manager가 감지 → 새 Pod 자동 생성
