...
> [AgentVigil: Generic Black-Box Red-teaming for Indirect Prompt Injection against LLM Agents](https://arxiv.org/abs/2505.05849) (Wang et al., UC Berkeley, EMNLP Findings 2025)
>
...
$$\text{score}(s) = \text{ASR}(s) + C \cdot \frac{\text{coverage_bonus}(s)}{\text{num_questions}}$$

기호 풀이:

- $$\text{ASR}(s)$$ = (성공한 공격 수) / (전체 시도 task 수). 이 시드가 **지금 당장 얼마나 잘 통하는가**.
- $$\text{coverage_bonus}(s)$$: 이전에 못 깨던 **새로운 task 조합**을 이 시드가 처음 깰 때마다 1씩 올라간다. "새 영역을 개척했는가"를 보상.
