...
2. **검증 불가능한 도메인(대화, 글쓰기, 안전성)에서는 갈라진다** — 하지만 그 분기의 한복판에서 **generative reward model(GRM)이 처음으로 프로덕션 report에 등장**했다. 이 시리즈 6부가 "연구 단계에만 있다"고 정리했던 흐름이 DeepSeek-V4에서 실제 학습 파이프라인으로 넘어온 순간이다.
