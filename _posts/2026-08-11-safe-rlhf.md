...
description: "RL Reward 설계 시리즈 #15 — helpful과 harmless를 분리하고, 안전성을 최대화가 아닌 제약으로 푼다"
categories: [paper]
...
[#59](/blog/2026/frontier-reward-design/)의 안전성 절에서 봤듯, 이 문제의 실전 함정은 **over-refusal(과잉 거절)** 이다. "거절하면 안전하다"고 단순하게 보상하면, 모델은 무해한 요청까지 거절하는 쪽으로 hacking된다. 안전을 세게 밀수록 도움됨이 무너지는 것이다.
...
그 결과 Alpaca-7B에서 유해 응답 비율을 **53.08% → 2.45%** 로 낮추면서 helpfulness Elo도 +244.91점 올렸다. 안전과 도움됨을 맞바꾸지 않고 **둘 다** 끌어올린 것이다. 이 글은 그 설계를 뜯는다. 같은 문제를 규칙으로 푸는 접근은 다음 편 [#16 Rule-Based Rewards](/blog/2026/rule-based-rewards/)에서 다룬다.
...
[#59](/blog/2026/frontier-reward-design/)에서 본 최신 모델들의 안전 설계가 이 논문의 후예다.
...
그리고 안전 명세를 학습된 cost model이 아니라 **규칙으로 옮기는** 또 하나의 갈래가 있다. 다음 편 [#16 Rule-Based Rewards](/blog/2026/rule-based-rewards/)가 그것이다.
...
한계도 분명하다. cost model 역시 학습된 근사이므로 그 자체가 hacking 대상이 될 수 있고([#10](/blog/2026/reward-model-overoptimization/)), "무엇이 안전인가"의 14개 범주 정의는 여전히 사람의 값 판단에 기댄다. 그래서 규칙으로 명세를 옮기는 [#16 RBR](/blog/2026/rule-based-rewards/), 그리고 [#59](/blog/2026/frontier-reward-design/)의 rubric·judge 기반 안전 설계가 그 뒤를 잇는다. 안전성 reward 설계는 결국 **"helpful과 harmless라는 두 신호를 어떻게 분리하고, 안전 압력을 어떻게 상황에 맞게 조절하느냐"**의 문제였다.
...
- Mu et al. (OpenAI), 2024. [Rule Based Rewards for Language Model Safety](https://arxiv.org/abs/2411.01111) — [#16](/blog/2026/rule-based-rewards/)에서 다룰 규칙 기반 안전 reward.
...
이 글은 RL Reward 설계 시리즈의 열다섯 번째 글이다.
