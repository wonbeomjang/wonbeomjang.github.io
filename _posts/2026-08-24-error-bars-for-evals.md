...
description: "LLM 평가 체계 시리즈 #18 — Miller의 다섯 통계 권고와 클러스터 SE, 분산 분해, IQM으로 5부를 닫는다"
categories: [paper]
...
[#15](/blog/2026/confidence-intervals/)에서 점수를 이항비율의 신뢰구간으로 다뤘고, [#16](/blog/2026/significance-testing/)에서 두 모델의 차이를 짝지어 검정하는 법을, [#17](/blog/2026/statistical-power/)에서 검정력과 다중비교를 봤다. 모두 일반 통계학의 도구였다. 이 글은 그 도구들을 LLM eval이라는 구체적 현장에 맞춰 재단한다. 지문 하나에 문항이 여러 개 딸려 있고, 같은 문항을 temperature를 올려 여러 번 샘플링하고, 시드 3\~5개로 벤치마크 스위트 전체를 요약해야 하는 현장이다. 5부([#15](/blog/2026/confidence-intervals/)\~#18)의 결론편이다.
...
| 프롬프트·포맷 | 템플릿 선택, 선택지 순서    | 여러 프롬프트 평균 + 분산 보고 ([#6](/blog/2026/mcqa-fragility/)) | 아니오                                         |
| 채점자        | judge 또는 사람 라벨러      | 라벨러 늘리기, PPI ([#19](/blog/2026/judge-statistics/))          | 아니오                                         |
...
표의 나머지 세 줄(프롬프트·포맷, 채점자, 클러스터 구조)은 이 K-분해와는 다른 축의 문제다. 프롬프트·포맷 분산은 [#6](/blog/2026/mcqa-fragility/)에서 이미 다뤘고, 채점자 분산은 [#19](/blog/2026/judge-statistics/)에서 Prediction-Powered Inference로 다룬다. 클러스터 구조는 이 글의 두 번째 권고에서 정면으로 다룬다.
...
**언제 쓰나.** 정확도 하나만 던지지 말고 항상 이 SE를 같이 보고한다. [#15](/blog/2026/confidence-intervals/)에서 다룬 Wilson 구간과 결합하면 $$n$$이 작거나 $$\bar s$$가 0·1에 가까운 극단 상황도 보정된다.
...
**언제 쓰나.** 거의 항상. 두 모델을 같은 문항셋으로 비교하는 상황이면 무조건 짝지어야 한다. [#16](/blog/2026/significance-testing/)에서 다룬 paired bootstrap·McNemar가 바로 이 원리를 구현한 도구다.
...
**정의.** eval을 만들기 전에, 원하는 최소 탐지 효과(MDE) $$\delta$$를 검출하는 데 몇 문항이 필요한지 미리 계산한다. [#17](/blog/2026/statistical-power/)의 표본크기 공식을 LLM eval의 짝짓기·클러스터·재샘플링 구조에 맞춰 다시 쓴 형태다.
...
**직관.** [#17](/blog/2026/statistical-power/)의 표본크기 공식과 같은 뼈대지만, 분자의 분산 항이 짝지은 차이의 분산($$\omega^2$$, 공분산이 이미 빠진 형태)과 디코딩 분산을 함께 담고 있다는 점이 다르다.
...
이 결론이 5부([#15](/blog/2026/confidence-intervals/)\~#18)에서 갖는 자리도 분명하다. [#15](/blog/2026/confidence-intervals/)가 "점수는 추정치다"라는 관점 자체를 세웠고, [#16](/blog/2026/significance-testing/)이 그 추정치들의 차이를 검정하는 법을, [#17](/blog/2026/statistical-power/)이 사전에 표본 크기를 계산하는 법을 다뤘다면, 이 글은 그 세 도구를 LLM eval의 실제 구조(클러스터, 디코딩 노이즈, 소수 시드)에 맞춰 다시 조립했다. 일반 통계학의 공식을 그대로 갖다 쓰면 클러스터 상관을 놓치고, 디코딩 노이즈와 문항 표집 분산을 혼동하고, 소수 시드에서 평균의 함정에 빠진다 — 이 글의 세 논문은 정확히 그 세 함정을 하나씩 막는다.
...
4. **프롬프트 고정 여부** — 하나의 템플릿으로 고정했는지, 여러 템플릿의 평균과 분산을 보고했는지([#6](/blog/2026/mcqa-fragility/)).
5. **표준오차와 그 계산 방식** — CLT 기반인지 클러스터 보정을 거쳤는지 명시한다. "SE는 대략 이 정도"라는 뭉뚱그린 서술로는 재현할 수 없다.
6. **짝지음 여부** — 두 모델을 비교할 때 같은 문항셋으로 짝지어 차이를 검정했는지([#16](/blog/2026/significance-testing/)).
7. **다중비교 보정 여부** — 벤치마크 여러 개, 서브태스크 여러 개를 동시에 비교했다면 Bonferroni나 BH로 보정했는지([#17](/blog/2026/statistical-power/)).

이 체크리스트를 다 채웠다고 해도 한 가지는 남는다 — 채점자 자체의 분산이다. 사람 라벨러든 LLM judge든, 채점자가 흔드는 분산은 지금까지의 어떤 권고로도 잡히지 않는다. 그 문제를 정면으로 다루는 것이 다음 글, [#19 judge를 통계로 다루기](/blog/2026/judge-statistics/)다.
